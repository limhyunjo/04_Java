...
public class FileService {
   ...
   */
	// String[] list()은 이름만 반환됨 File[]   listFiles() 지정된 파일을 다 가져옴
    ...
    */
	...
	public void method1() {
	  ...
	  if(!directory.exists()) {//폴더가 존재하지 않으면
		 ...
		 System.out.println(directory.getName());//20240215 파일 이름만
...
public void method2() {
	...
	//Invalid escape sequence 유효하지 않음
  
   try {
     if(!file.exists()) {//존재하지 않으면
	 
	    if(file.createNewFile()) {//파일 생성이 성공한 경우
		//Unhandled exception type IOException /예외 처리가 안되어있음
   ...
   // 향상된 for문  ->  for( 요소 저장 변수: 배열|컬렉션 ){}
	...
	// System.out.println(f.toString());	
	 // String File.toString() : 지정된 파일/디렉토리의 절대경로 
		 ...
		 * \workspace\04_JAVA\11_io\.settings 
		 * \workspace\04_JAVA\11_io\bin
		 * \workspace\04_JAVA\11_io\src
		 */
	...
	// System.out.println(lastModified);
	 ...
	 SimpleDateFormat sdf
	  = new SimpleDateFormat("yyyy-MM-dd a h:mm");// 2024-02-15 오전 9:16
