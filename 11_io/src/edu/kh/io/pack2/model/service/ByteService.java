...
/**********************************************************************/	
//IO관련 코드는 IOException이 발생될 가능성이 높다!
...
public void fileByteOutput() {
	...
	FileOutputStream fos = null;//try, finally구문에서 사용 예정
...
public void bufferdFileByteOutput() {
	 ...
	 */
	 ...
	 /* BufferedOutputStream / BufferedInputStream (필수로 알아야 함)
	  * 
	  * -FileOutputStream은 1byte씩 데이터 입출력
	  * 
	  ...
	  * 
	  */
	FileOutputStream fos = null;// try, finally 에서 사용 예정	 
 ...
 try {
	...
	bos = new BufferedOutputStream(fos);
		...
		long startTime = System.nanoTime();
	   bos.write(content.getBytes());//String을 byte 배열로 바꿔서 
...
public void fileByteinput() {
  ...
  try {
	  ...
	  //방법 1. 파일 내부 내용을 1byte씩 끊어져서 가져오기
	  ...
	  while(true) {//무한 반복
		  ...
		  sb.append((char)value);//읽어온 값을 char로 강제 형변환해서 글자 형태로 추가
...
public void fileByteinput2() {
 ...
 try {
	  ...
	  byte[] bytes = fis.readAllBytes();// 얻어옴
 ...
 /**
 * bufferedIniutStream 보조 스트림을 이용한 성능 향상
 */
	...
	try {
	 ...
	 // 경로 입력 받기
	   ...
	   //해당 경로에 파일이 존재하는지 확인
		...
		//System.out.println("존재 여부 확인 :" + file.exists());  
	 ...
	 // ------------------------------------------------------------
	  ...
	  // taget : /io_test/20240215/노래가사.txt  
