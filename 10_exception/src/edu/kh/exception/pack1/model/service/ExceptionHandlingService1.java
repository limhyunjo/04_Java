...
public void method4() {
	...
	//java.util.InputMismatchException(입력 값이 맞지 않음)
 ...
 * ==부모 예외만 실행되고 반복됨
 * -> 부모 예외는 아랫쪽에 작성해야 컴파일 에러가 뜨지 않는다
 */
public void method5() {
	int input = 0; 
	...
	do {
		...
		/*
		 * }catch(RuntimeException e) { System.out.println("---실행 중 발생한 예외 처리 완료---"); }
		 */
		
		//부모 타입 예외 참조 변수 = 자식 타입 예외 객체
		//-> 다형성 업캐스팅 (매개 변수)
		// = new ArithmeticException()
	    // 자식 예외를 부모catch 문에서 모두 잡아서 처리하기 때문에
		// 자식 catch문 까지 코드가 도달할 수 없다(수행 불가, 죽은 코드)
		
		//-> 해결 방법 : 부모 catch를 자식 catch밑에 작성!
		...
		//Unreachable catch block : 도달할 수 없는 catch 구문
		 ...
		 input=-1; // 첫 반복에서 종료되지 않게 input 값을 수정하는 코드를 써준다
	  }catch(NullPointerException e) {
		  //객체로 만들어서 매개 변수로 쓰고 자식 객체로 부모 타입 참조 중
...
public void method6() {
   ...
   try {
	   //각종 예외 강제 발생 (Illegal :잘못된, Argument : 전달인자 / 값이 잘못됨)
