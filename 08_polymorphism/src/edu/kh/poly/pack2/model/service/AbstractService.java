...
public void method1() {
 ...
 // Animal a= new Animal();
  //Animal cannot instance(객체화) the type Animal
...
//	Animal[] arr = new Animal[4];-> 참조 변수 4개 만들어짐
	...
	for(Animal a :arr) {
		//참조 변수 a 안에는 객체의 주소가 저장됨
		System.out.println("--------------------------------------");
		
		//아래 구문들 모두 동적 바인딩 / 수행되어서 실제 참조하는 자식 객체가 나옴
		//System.out.println(a.toString()); toString 안 써도 자동으로 해줌
		System.out.println(a);
       //Object.toString()목적 : 객체를 문자열로 표현
	   //                 == 객체의 필드를 모두 모아 하나의 문자열로 반환
		
	   //print() 관련된 구문에 참조 변수를 작성하면
	   //자동으로 toString() 메서드를 호출 하도록 되어 있다
		a.eat();
        a.sleep();
	}
