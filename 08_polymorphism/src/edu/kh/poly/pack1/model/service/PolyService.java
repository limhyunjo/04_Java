...
public class PolyService {
  ...
  * -> 자식 객체, 부모 객체, Object객체 등으로 변하는 것처럼 보임
  * 
  * 2. 오버 로딩
  * -> 같은 클래스에서 같은 이름의 메서드를 여러번 작성하는 기술
  * -> 조건 : 메서드명 동일, 매개변수가 개수, 타입, 순서 하나라도 달라야 함
  * 
...
public void method1() {
	...
	//-> 원래는 컴퓨터 값 처리 원칙에 위배(자동 형변환 대상X)
	//-> 자식 객체 내부에 존재하는 
   ...
   //자식 기능 호출 -> 가능 O/ [ X ]
 //  s1.setIosVersion(17);
 //  s2.setAndroidVersion(14); 자식 객체 참조 안됨
 ...
 * 
 * -업캐스팅된 객체( 자식 -> 부모 )를 참조하던 
 * 부모 타입 참조 변수를 
 * 자식 타입으로 강제 형변환 하여 
 * 다시 자식 객체를 참조하도록 변경(부모 -> 자식) ==다운 캐스팅
 * 
 * 업캐스팅을 먼저해야 다운 캐스팅 가능
 */
public void method2() {	
 ...
 *   -> 오버라이딩된 메서드 호출 시 
 *   (이름이 같은 메서드 2개==부모 자식이 서로 같은 이름의 메서드를 가지고 있음)
 *   
 *   실제 참조하고 있는 자식 타입 객체의 메서드가 우선권을 갖는다!!
 */
public void method4() {
...
public void method5() {
  ...
  //스마트 폰 자료형의 배열 타입인 arr의 SmartPhone 참조 변수 3개 생성
  
  //배열의 각 요소(SmartPhonr 참조 변수)에
  ...
  //-> 위 셋의 자료형이 다 다름 == 많은 코드를 쉽게 관리 가능
  
  // 객체 배열에 다형성을 적용함으로써
  // 배열 하나로 다양한 자료형을 묶음으로 다룰 수 있게 됨
  //-> 변수 생성 코드 감소 + for문 (반복문) 으로 한번에 제어 가능
  //-> 전반적으로 코드 길이 감소
...
public void method6() {
 ...
 public void printToString(SmartPhone s) {//아래에서 주소를 전달 받음
	 // SmartPhone 부모 참조 변수
	 //자식 객체의 참조 변수가 부모 매개 변수에 저장 (업캐스팅이 적용)
	 
	 //매개 변수가 부모 타입이라 업캐스팅 적용
 ...
 * 
 * ->참조 변수가 참조하는 객체(instance)가
 *   지정된 클래스로 만들어진 객체가 맞아?
...
public void method8(){ 
 ...
 /**
 * 객체 배열 + 업캐스팅 + 다운 캐스팅 + instance of
 */
public void method9() {
	 
 // 부모 타입 객체 배열 생성
	//-> SmartPhone 참조 변수 3칸 짜리 배열 생성
 SmartPhone[]arr= new SmartPhone[3];
 ...
 //+메서드 반환형에 다형성(업캐스팅) 적용
  for (int i=0; i<arr.length; i++) {//순차 접근== 반복 접근
	
	arr[i]=createPhone(i+1);
	
	// arr[0] = createPhone(1) == new SmartPhone();
	// arr[1] = createPhone(2) == new Iphone();
	//smarphone 참조변수에 iphone 객체를 집어넣음==업캐스팅 자식이 부모로 변함
  ...
  //향상된 for문 + 매개 변수 다향성(업캐스팅) + 동적 바인딩
  for(SmartPhone s : arr) {
	  printToString(s);
  }
