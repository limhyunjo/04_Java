...
public class StudentView {
	...
	public void displayMenu() {
		...
		do {
			...
			case 5: updateStudent(); break; // update가 수정한다는 뜻임
	...
	private void selectIndex() {
		...
		// Service로 index 전달 후 일치하는 학생이 있으면 반환받기
		Student std =service.selectIndex(index);
		
		if(std ==null) 
			System.out.println("해당 인덱스에 학생이 존재하지 않습니다");
		else {         System.out.println("학번 :" + std.getStudentNumber());
		
		String temp = null; //반환 받아오면 아래가 출력
	...
	private void selectName() {
		...
		String name = sc.next();// 이름을 하나 입력 받겠다
		
		// 서비스 클래스로 전달해서 일치하는 이름을 가져온다
	...
	private void updateStudent() {
		...
		Student std =service.selectIndex(index);// 이미 썼던 것 재사용
		//얕은 복사로 가져와서 원본 수정 가능
		
		if(std==null) // 학생 호출
			{System.out.println("해당 인덱스에 학생이 존재하지 않습니다");
		return;
			}
		//점수를 한번에 입력받아 바로 std에 세팅
	...
	private void selectScore() {
		...
		System.out.print("조회할 학생 인덱스 입력:");
		int index = sc.nextInt();
		...
		String result = service.selectScore(index);// 호출해서 문자열로 반환받기
