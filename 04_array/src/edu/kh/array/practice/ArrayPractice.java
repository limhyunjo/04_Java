...
public class ArrayPractice {
 ...
 Scanner sc = new Scanner(System.in);
	...
	public void practice3() {
		...
		// 1) Stack 영역에 int[]을 참조하기 위한 참조형 변수 arr선언
	...
	public void practice5() {
        ...
        int len = sc.nextInt();
        //정수를 입력받아 변수 len에 저장
        //nextInt();메서드는 정수 값을 입력받는데 사용
		
		int[] arr = new int[len];//새 배열 생성 내가 넣는 정수 만큼의 길이
	...
	public void practice6() {
		...
		for(int i=0, len =input.length();i<len  ;i++) {// input.length 해도 되지만 효율
	...
	public void practice7() {
		...
		//반복
		 for(int i=0; i<len;i++) {
			 ...
			 arr[i]=count; //count 값을 배열에 대입
		 }
		 
         // Arrays ctrl + space하면 위에 import생김
		// Arrays.toString(배열명) 
	...
	public void practice8() {
		int[] randoms= new int[10];//10칸 짜리 난수 배열 만듬
	...
	public void practice9() {
		...
		for(int i=0; i<randoms.length; i++) {
			...
			if(i==0) {//첫 반복인 경우 ->첫 번쨰 난수를 비교 기준으로 삼음
				max=randoms[i];
				min=randoms[i];
			  continue;//다음 반복으로 이동
	...
	public void practice10() {
		...
		for(int i=0; i<randoms.length; i++) {
			...
			// 앞쪽 인덱스에 존재하면 
			// 다시 현재 인덱스에 새로운 난수를 새성해서 대입
			//(중복이 없을 때까지 반복) ->for문
			
			//현재 인덱스 앞쪽 까지만 순차 접근하는 for문 작성
			for(int x=0; x<i; x++) {
				
			// 현재 인덱스에 저장된 값과 (지금 발생한 난수)
	...
	public void practice11() {
		...
		for(int i=0; i<lotto.length-1; i++) {//칸 지정(비교 주체)
			for(int x=i+1; x<lotto.length; x++) {//비교 대상 지정
				...
				//위 if문의 부등호 방향에 따라 오름차순 내림차순 정렬이 달라짐
					int temp =lotto[i];
				    lotto[i]=lotto[x];
				    lotto[x] = temp;
				}
