...
public class SetService {
	 ...
	 * Set인터페이스의 자식 클래스
	 * 
	 * 1. HashSet(대표적으로 많이 사용) : 처리 속도가 빠른 Set
	 * 
	 * 2.LinkedHashSet: 순서를 유지하는 Set
	 * 
	 * 3.TreeSet :자동 정렬되는 Set (자동으로 오름차순, 내림차순 정렬 가능)
	 */
	...
	public void method1() {
	 ...
	 //HashSet 객체 생성
	  ...
	  //1 boolean add(E e) :추가
		...
		System.out.println(set); //set.toString( ) 자동으로 됨
		//-> HashSet 순서 유지 X 확인
	  ...
	  //전달받은 e 가 Set에 있으면 true, 없으면 false
...
public void method2() {
	...
	while(it.hasNext()) {
	 ...
	 //E Iterator.next(): 다음 요소를 꺼내와 반환 hasNext()와 같이 씀
	...
	//List는 Iterator와 모습은 같이 나오지만 하나씩 꺼내 쓸 수 있음
...
public void method3() {
	...
	for(Person p : personSet) {
		System.out.println(p); //25세 홍길동이 한 번만 출력(중복 제거)
	}
...
public void method4() {
 ...
 //2) Random.nextInt() 
	...
	// random.nextInt(45) :  0 <= x < 45 난수
		lotto.add(random.nextInt(45)+1); //1~45사이의 난수 추가+중복 제거
	}
...
public void lottoNumberGenerator() {
	...
	Random random = new Random();
	//Set하나당 로또 한묶음
	//integer만 저장하는 set을 list에 저장한다
	//위의 Set에 저장된 로또 번호 묶음을 저장할 List
	List<Set<Integer>>lottoList = new ArrayList<Set<Integer>>();
	

	// 초기식 조건식 증감식
	for(int i=1; i<=num ; i++) {
	//초기식 end = amount/1000; 조건식 i<end	
		
		//for문 반복될 때 마다 새로운 Set객체를 생성
		Set<Integer> lotto = new TreeSet<Integer>();
		...
		while(lotto.size()<6) {
		   ...
		   // 계속 새롭게 생성되어서 Set애 저장되기에 clear 필요 없다
