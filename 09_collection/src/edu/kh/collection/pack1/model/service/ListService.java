...
//특징
//1) 크기 제한 없음 (크기가 부족하면 자동으로 증가)
//2) 추가, 수정, 삭제, 검색 등 다양한 기능이 구현되어 있음
//3) 객체만 저장 가능
//   ( 기본 자료형은 안됨.....하지만 Wrapper클래스를 이용하면 됨!!!)
public class ListService {
	...
	public void method1() {
	 ...
	 //List list = new ArrayList();//기본 생성자로 만들면 기본 10칸 짜리 생성됨
	 List list = new ArrayList(3);// 3칸 짜리 배열 비슷한거 생성
	 
	 // 반환형 메서드명(매개변수)
	 
	 // E(Element): 요소를 뜻하는 상징적인 글자(자료형 X)
	 //             == Oject, String, Student 등
	 //				   자료형 가리지 않고 들어갈 수 있다
	 
	 //1. boolean add(E e) : 마지막 요소로 추가 
	  
	 //자료형 다 다르게 넣어봄 => list는 Object 배열이라 아무거나 다 넣을 수 있다
		list.add("아무거나");  
		list.add(new Object());
		...
		for(int i=0; i< list.size(); i++) {
			...
			//List의 i번째 요소가 double(->Double(class) 변환)인 경우
			...
			//Object는 모든 class의 super class여서 모든 자식 객체 참조 가능
	...
	public void method2() {
	 ...
	 //* Collection을 쓸 때는 앞으로 무조건 제네릭을 쓴다
	...
	// System.out.println(strList.toString());
	 //위에서 toString을 안써도 됨 -> 참조 변수만 써도 호출 가능
