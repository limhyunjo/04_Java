package edu.kh.collection.pack4.model.sevice;
...
public void method1() {	
	...
	// Integer wrap1= new Integer(num1); 
	 ...
	 Integer wrap1 = num1; //컴파일러가 Auto Boxing 수행 코드를 자동으로 추가해줌
	 					   //(int) num1 -> (Integer) num1
	 
	 int num2= wrap1.intValue();// 정수값 반환(추천 X)
	 int num3 = wrap1; //컴파일러가 Auto UnBoxing 수행 코드를 자동으로 추가해줌
 ...
 * -> 대부분이 static!!!
 *  static : 정적 메모리 영역에 존재 한번 생기면 영원히 존재, 프로그램 실행 시 가장 먼저 읽힘
 *  -> 클래스명.필드명 / 클래스명.메서드명() 사용
 */
public void method2() {
	
	// 정수형, 실수형 공통으로 사용 가능
	 System.out.println("byte의 용량 :" +Byte.BYTES +"바이트");//1
	 System.out.println("byte의 용량 :" +Byte.SIZE + "비트");//8
    ...
    //-----------------------------------------------------------------
	 ...
	 //단, Char(Character)는 별돋로 존재하지 않음
	 ...
	 String change3 = test3 + ""; //long + String -> String + String 으로 이어쓰기
	 //이어쓰기 시에 하나의 String으로 변하고 Double.valueOf(test2)가 자동으로 호출됨
 }
...
public void method4() {
 ...
 String temp2 = "Hello!!";//0X400 주소만 반환 받음 / 기존 객체 활용 (얕은 복사)
 ...
 //객체의 필드 값(저장된 값)을 비교
 System.out.println("주소를 비교:" + (temp1==temp2));//equals 보다 속도가 빠름
 //변수에 저장된 값 (주소) 비교
 //같은 리터럴 참조여서 그냥 비교 가능
...
public void method5() {
  ...
  // 저장되는 값은 같은데 셋 다 주소가 다름 == "abcd"를 재활용 하지 않음
  //-> 값은 같지만 전부 다른 객체
 }
 ...
 *    
 * -StringBuilder :Thread Safe 미제공 (비동기, 속도가 조금더 빨라서 추천)
